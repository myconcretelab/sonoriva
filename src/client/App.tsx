...
export default function App() {
  ...
  const [editingCategoryBackground, setEditingCategoryBackground] = useState<Category>();
  ...
  return <div className={`app-shell ${remote ? 'remote-mode' : ''}`}>
    ...
    <main className="workspace">
      ...
      {detail && <section className="category-strip">
        ...
        <div className="category-tabs-row" style={{ '--category-tab-width': `${categoryWidth}px` } as React.CSSProperties}>
          <div className="category-strip-tools" role="group" aria-label="Gestion des catégories"><button className={`icon-button subtle category-manage-toggle ${categoryManageMode ? 'active' : ''}`} onClick={() => { setCategoryManageMode((current) => !current); setSelectionMode(false); setSelectedTrackIds(new Set()); setDraggedCategoryId(undefined); setDropCategoryOrderId(undefined); setDropCategoryAfter(false); }} aria-label={categoryManageMode ? 'Terminer la gestion des catégories' : 'Gérer les catégories'} title={categoryManageMode ? 'Terminer' : 'Réordonner ou supprimer'}><ArrowUpDown size={16} /></button><button className="icon-button subtle" onClick={createCategory} aria-label="Nouvelle catégorie"><Plus size={17} /></button></div>
        ...
        <div className="dashboard-actions">
          ...
          <div className="dashboard-control">
            ...
            {columnsOpen && <div className="dashboard-popover columns-popover">
              ...
              <label><span>{soundboardView === 'list' ? 'Colonnes de liste' : 'Colonnes de cartes'}</span><strong>{trackColumns}</strong><input type="range" min={soundboardView === 'list' ? 1 : compactLayout ? 1 : 2} max={soundboardView === 'list' ? compactLayout ? 2 : 8 : compactLayout ? 3 : 12} value={trackColumns} onChange={(event) => updateTrackColumns(Number(event.target.value))} /></label>
    ...
    {editingCategoryBackground && <CategoryBackgroundDialog category={editingCategoryBackground} onClose={() => setEditingCategoryBackground(undefined)} onChanged={() => { setEditingCategoryBackground(undefined); void refreshProject(); }} />}
